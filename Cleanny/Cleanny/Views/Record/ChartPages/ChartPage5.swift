...
struct ChartPage5: View {
  ...
  //  @EnvironmentObject var MonthData: MonthDataStore
    ...
    var body: some View {
        ...
        BarChartView(data: ChartData(values:getChartData(tempMonthArr: monthData)), title: "설거지" )
            .padding()
    }
    func getChartData(tempMonthArr: FetchedResults<MonthHistory>)->Array<(String,Double)>{
        ...
        while i >= 0 {
            if(i>5){
                ...
                tempMonthArr[4 * 12 + tempMonth].cleaningCount = 0
            }
            else{
                tempMonth = (currentMonth - 1)
                tempMonth -= i
                ...
                chartData.append(("\(tempMonthArr[4 * 12 + tempMonth].monthName)",Double(tempMonthArr[4 * 12 + tempMonth].cleaningCount)))
            }
        }
